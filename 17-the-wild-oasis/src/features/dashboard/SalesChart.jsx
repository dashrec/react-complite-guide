...
const StyledSalesChart = styled(DashboardBox)`
  grid-column: 1 / -1; // this layout goes from the beginning to the end
...
function SalesChart({ bookings, numDays }) {
  ...
  // it will gives us new object out of allDates which will have additional totalSales & extrasSales for a given day
  const data = allDates.map((date) => {
    return {
      label: format(date, 'MMM dd'),
      totalSales: bookings
        .filter((booking) => isSameDay(date, new Date(booking.created_at))) // this func works only with javascript dates therefore we need to convert string to js date
        .reduce((acc, cur) => acc + cur.totalPrice, 0), // totalSales for this day
